    ...
    if (!highlightDB) {
        highlightDB = await openDB<Highlights>("highlights", 1, {
            upgrade(db) {
                ...
                highlightStore.createIndex("collection, book, chapter, verse", ["collection", "book", "chapter", "verse"])
                highlightStore.createIndex("collection, book, chapter", ["collection", "book", "chapter"])
            },
        });
    }
    return highlightDB;
