...
import { generateHTMLTest, isBibleBook, getReferenceFromString } from './scripture-reference-utils';
...
describe('Scripture Reference Utilities', () => {
    ...
    describe('getReferenceFromString', () => {
        it('gets book from string', () => {
            const [collection, book, fromChapter, toChapter, verseRanges]  = getReferenceFromString('MAT 1:1-25');
        ...
        it('gets handles chapter only reference', () => {
            const [collection, book, fromChapter, toChapter, verseRanges]  = getReferenceFromString('MAT 1');
            const [fromVerse, toVerse, separator] = verseRanges[0];
            expect(toChapter).toEqual(1);
