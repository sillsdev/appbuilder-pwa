...
test('Test Text Appearance', async ({ page }) => {
    ...
    for (let thm of cfg.default.themes) {
        if (thm.name == 'Normal' && thm.enabled == true) {
            ...
            await clbton.click();
            await expect(page.locator('.dy-drawer-content')).toHaveCSS(
                'background-color',
                'rgb(240, 240, 240)'
            );
        }
...
test('Test Settings', async ({ page }) => {
    ...
    if (await page.getByTestId('settings-icon').isVisible()) {
        ...
        if (cfg.default.programType == 'SAB') {
            ...
            const selbox2 = page
                .getByRole('combobox')
                .filter({ has: page.getByRole('option').filter({ hasText: 'Show images' }) });
            ...
            const selbox3 = page
                .getByRole('combobox')
                .filter({ has: page.getByRole('option').filter({ hasText: 'Show normal size' }) });
