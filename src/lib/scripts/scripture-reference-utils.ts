...
export function getDisplayString(collection: string, bookId: string, chapter: number,  ranges: [number, number, string]) {
    const chapterVerseSeparator = config.bookCollections.find((x) => x.id === collection).features[
        'ref-chapter-verse-separator'
    ];
    const verseRangeSeparator = config.bookCollections.find((x) => x.id === collection).features[
        'ref-verse-range-separator'
    ];
    const bookName =
        config.bookCollections
            .find((x) => x.id === collection)
            .books.find((x) => x.id === bookId)?.name || bookId;
    return bookName + ' ' + item.chapter + chapterVerseSeparator + item.verse;
}