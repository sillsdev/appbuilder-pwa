<script>
    ...
    $: currentIndex = wordsList.findIndex(word => {
        ...
        if (typeof word === 'object') {
            ...
            } else if (word.name && currentWord.word && word.homonym_index !== undefined && currentWord.homonym_index !== undefined) {
                // For vernacular words with homonyms, match both word and homonym index
                return word.name === currentWord.word && word.homonym_index === currentWord.homonym_index;
...
<div class="flex items-center justify-between bg-[#e1bee8] p-2">
    ...
    >
        <div class="w-0 h-0 border-y-[12px] border-y-transparent border-r-[20px] border-r-gray-700"></div>
    </button>
    ...
    >
        <div class="w-0 h-0 border-y-[12px] border-y-transparent border-l-[20px] border-l-gray-700"></div>
    </button>
</div>