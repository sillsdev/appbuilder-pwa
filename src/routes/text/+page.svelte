<script lang="ts">
    ...
    const prevSettings = $derived({     // save the info to load in the previous page settings
        ...viewSettings,
        references: {
    ...
    const nextSettings = $derived({     // save the info to load in the next page settings
        ...viewSettings,
        references: {
            ...viewSettings.references,
...
<svelte:window bind:innerWidth bind:innerHeight onpointermove={handleMouseMove} onpointerup={handleMouseUp} />
    ...
    >
        ...
        <div class="md:flex md:flex-row mx-auto justify-center" style:direction={$direction}>
            ...
            >
                ...
                <div class="p-2 w-full" style="position: absolute; left: {-draggableWidth}px; clip-path: inset(0 0 0 {draggableWidth - x.current}px);">
                    <main>
                        <div class="max-w-screen-md mx-auto">
                            {#if format === 'html'}
                                <HtmlBookView {...prevSettings as HtmlBookViewProps} />
                            {:else}
                                <ScriptureViewSofria {...prevSettings as ScriptureViewSofriaProps} />
                            {/if}
                        </div>
                    </main>
                </div>

                <div class="p-2 w-full" style="position: absolute; clip-path: inset(0 {x.current}px 0 {0-x.current}px);">
                    <main>
                        <div
                            aria-hidden="true"
                        ...
                        >
                            ...
                            {:else}
                                <ScriptureViewSofria {...viewSettings as ScriptureViewSofriaProps} />
                            {/if}
                        </div>
                    </main>
                </div>

                <div class="p-2 w-full" style="position: absolute; left: {draggableWidth}px; clip-path: inset(0 {draggableWidth + x.current}px 0 0);">
                    <main>
                        <div class="max-w-screen-md mx-auto">
                            {#if format === 'html'}
                                <HtmlBookView {...nextSettings as HtmlBookViewProps} />
                            {:else}
                                <ScriptureViewSofria {...nextSettings as ScriptureViewSofriaProps} />
                            {/if}
                        </div>
                    </main>
                </div>
