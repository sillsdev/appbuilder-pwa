<script lang="ts">
    ...
    const SETTINGS_CATEGORY_NAVIGATION = 'Settings_Category_Navigation';
    const SETTINGS_CATEGORY_NOTIFICATIONS = 'Settings_Category_Notifications';
    const SETTINGS_CATEGORY_AUDIO = 'Settings_Category_Audio';
    const SETTINGS_CATEGORY_TEXT_DISPLAY = 'Settings_Category_Text_Display';

    function initUserPreferenceSettings(): Array<App.UserPreferenceSetting> {
        ...
        if (config.mainFeatures['settings-show-border'] && config.traits['has-borders']) {
            ...
            settings.push({
                ...
                key: 'show-border',
                defaultValue: $userSettings['show-border']
            });
        }
    ...
    let settings = initUserPreferenceSettings();
    let categories = [];
    [
